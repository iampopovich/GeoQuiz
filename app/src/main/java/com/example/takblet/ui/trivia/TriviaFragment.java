...
public class TriviaFragment extends Fragment {
  ...
  public View onCreateView(
    ...
    TriviaAdapter adapter =
        new TriviaAdapter(getContext(), triviaViewModel.getQuestionList().getValue());
    binding.recyclerView.setLayoutManager(new LinearLayoutManager(getContext()));
    binding.recyclerView.setAdapter(adapter);
    triviaViewModel
        .getQuestionList()
        .observe(getViewLifecycleOwner(), questions -> adapter.notifyDataSetChanged());
  ...
  private void fetchQuestion() {
    ...
    StringBuilder url =
        new StringBuilder("https://opentdb.com/api.php?")
            .append("amount=")
            ...
            response -> {
              try {
                ...
                for (int i = 0; i < results.length(); i++) {
                  ...
                  triviaViewModel.addQuestion(
                      new TriviaQuestion(
                          Html.fromHtml(result.getString("question"), Html.FROM_HTML_MODE_COMPACT)
                              .toString(),
